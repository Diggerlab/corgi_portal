...
task "create:service_functions" => :environment  do
  ...
  user_api = ["#{GAME_API}/users/new,方法:get", "#{GAME_API}/login,方法:get", "#{GAME_API}/users/:token/change_password,参数：token, 方法:get ",
   "#{GAME_API}/password_requests/new,方法:get", "#{GAME_API}/password_requests/new_password,方法:get"]
  ...
  build_versions_api = ["#{GAME_API}/build_versions.json,参数:publisher 游戏号 build 版本号，方法:get"]
  build_versions_instructions = ["通过调用API接口可以使游戏用户更新游戏"]

  notices_titles = ["获取公告(多语言)"]
  notices_api = ["#{GAME_API}/notices.json, 参数:lag(en,ja,zn),方法:get"]
  notices_instructions = ["通过调用API接口获取公告信息"]

  friends_titles = ["获取好友列表", "导入好友"]
  friends_api = ["#{GAME_API}/friends.json, 参数:token,方法:get", "#{GAME_API}/friends/import, 参数:token 用户 token :email 用户 email, :code 用户唯一 code, 方法:post"]
  friends_instructions = ["通过调用API接口获取好友列表", "通过调用API接口导入新浪好友"]

  recharges_titles = ["获取iap产品列表", "购买 iap 产品回调接口"]
  recharges_api = ["#{GAME_API}/recharges.json, 参数:token,方法:get", "#{GAME_API}/recharges.json, 参数:token,方法:post"]
  recharges_instructions = ["通过调用API接口获取好友列表", "通过调用API接口导入新浪好友"]


  feedbacks_titles = ["反馈", "回复"]
  feedbacks_api = ["#{GAME_API}/feedbacks/new, 参数:token,方法:get", "#{GAME_API}/feedbacks/feedback_id/responses/new, 参数:token,:feedback_id回复feedback的id,方法:get"]
  feedbacks_instructions = ["通过调用API接口用户可以提交反馈", "通过调用API接口可以进行回复用户"]

  sessions_titles = ["qq账号登录", "新浪账号登录"]
  sessions_api = ["#{GAME_API}/sessions/qq_callback, 参数:weibo_access_token:qq 登录成功后返回的 Access Token;client_id: your app_id;open_id: your open_id; expires_in_sec:Access Token 将要过期的秒数,方法:post",
   "#{GAME_API}/sessions/sina_callback, 参数:weibo_access_token:sina 登录成功后返回的 Access Token; expires_in_sec:Access Token 将要过期的秒数,方法:post"]
